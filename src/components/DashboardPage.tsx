    ...
    <div id="dashboard-page" className="max-w-7xl mx-auto px-6 py-10 space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-brand-card border border-slate-200 p-5 rounded-2xl flex items-center space-x-4">
          ...
          <div>
            <div className="text-xs text-slate-500 font-mono uppercase">Overall Grade</div>
        ...
        <div className="bg-brand-card border border-slate-200 p-5 rounded-2xl flex items-center space-x-4">
          ...
          <div>
            <div className="text-xs text-slate-500 font-mono uppercase">Profile Alignment</div>
            <div className="text-xl font-display font-bold text-slate-800 mt-1">
              {analysisResult ? `${analysisResult.crossReference.alignmentScore}%` : "No Data"}
            </div>
            {analysisResult && <span className="text-[10px] text-slate-500 font-mono">Resume vs GitHub</span>}
          </div>
        </div>
        ...
        <div className="bg-brand-card border border-slate-200 p-5 rounded-2xl flex items-center space-x-4">
          ...
          <div>
            <div className="text-xs text-slate-500 font-mono uppercase">Active Portfolio</div>
            <div className="text-sm font-display font-bold text-slate-800 mt-1 truncate max-w-[150px]">
              {studentProfile.resumeFileName ? studentProfile.resumeFileName : "No Resume Uploaded"}
            </div>
            <span className="text-[10px] text-slate-500 font-mono">Parsed PDF</span>
          </div>
        </div>
        ...
        <div className="bg-brand-card border border-slate-200 p-5 rounded-2xl flex items-center space-x-4">
          ...
          <div>
            <div className="text-xs text-slate-500 font-mono uppercase">GitHub Repos</div>
            <div className="text-xl font-display font-bold text-slate-800 mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-8">
          {scorecard ? (
            <div className="bg-brand-card border border-slate-200 p-6 rounded-2xl space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-display font-bold text-slate-800">Latest Performance Assessment</h3>
                  <p className="text-xs text-slate-500 font-mono uppercase mt-0.5">Recorded Report Timeline</p>
                </div>
                <button
              ...
              <div className="p-5 bg-brand-bg border border-slate-200 rounded-xl grid grid-cols-1 sm:grid-cols-12 gap-6 items-center">
                <div className="sm:col-span-4 flex flex-col items-center">
                  <div className="relative w-24 h-24 flex items-center justify-center">
                    ...
                    <span className="absolute font-display font-bold text-xl text-slate-800">
                      {scorecard.overallScore}%
                    </span>
                  </div>
                  <div className="text-[10px] font-mono text-slate-500 uppercase mt-2">{scorecard.candidateLevel}</div>
                </div>

                <div className="sm:col-span-8 space-y-3">
                  <div>
                    <span className="text-[10px] font-mono text-slate-500 uppercase">Executive Summary</span>
                    <p className="text-sm text-slate-600 leading-relaxed mt-1">{scorecard.finalVerdict}</p>
                  </div>
            ...
            <div className="bg-brand-card border border-slate-200 p-8 rounded-2xl text-center space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="text-lg font-display font-semibold text-slate-800">No Mock Interviews Logged</h3>
              ...
              <button
                onClick={() => onNavigate("analyze")}
                className="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-800 font-medium text-sm rounded-xl transition-all inline-flex items-center space-x-2 border border-slate-200"
              >
          ...
          <div className="bg-brand-card border border-slate-200 p-6 rounded-2xl space-y-4 shadow-sm bg-slate-50/50">
          ...
          <div className="bg-brand-card border border-slate-200 p-6 rounded-2xl space-y-6">
            <div>
              <h3 className="text-lg font-display font-bold text-slate-800">The Mock Interview Roadmap</h3>
              <p className="text-xs text-slate-500 font-mono uppercase mt-0.5">Step-by-Step Training Program</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="space-y-3 p-4 bg-slate-100 rounded-xl border border-slate-200">
                ...
                <h5 className="font-display font-semibold text-slate-800 text-sm">Portfolio Scan</h5>
                <p className="text-xs text-slate-600 leading-relaxed">We sync with GitHub and extract skills from your resume to compile your baseline developer context.</p>
              </div>
              <div className="space-y-3 p-4 bg-slate-100 rounded-xl border border-slate-200">
                ...
                <h5 className="font-display font-semibold text-slate-800 text-sm">Adaptive Q&A</h5>
                <p className="text-xs text-slate-600 leading-relaxed">Gemini generates 6 custom questions grading you from warm-up basics up to core architectural tradeoffs.</p>
              </div>
              <div className="space-y-3 p-4 bg-slate-100 rounded-xl border border-slate-200">
                ...
                <h5 className="font-display font-semibold text-slate-800 text-sm">Score Assessment</h5>
                <p className="text-xs text-slate-600 leading-relaxed">Receive instant grades on vocabulary, pacing, filler-word counters, and technical depth metrics.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-brand-card border border-slate-200 p-5 rounded-2xl space-y-5">
            <h4 className="text-xs font-mono text-slate-500 uppercase tracking-wider pb-3 border-b border-slate-200">
            ...
            {analysisResult ? (
              <div className="space-y-4">
                <div>
                  <span className="text-[10px] font-mono text-slate-500 uppercase">Audited Student Name</span>
                  <div className="text-sm font-semibold text-slate-800 mt-0.5">{analysisResult.parsedResume.name || "Student User"}</div>
                </div>

                <div>
                  <span className="text-[10px] font-mono text-slate-500 uppercase">GitHub Anchor</span>
                  <div className="text-sm font-mono text-brand-primary mt-0.5">@{studentProfile.githubUsername}</div>
                </div>

                <div>
                  <span className="text-[10px] font-mono text-slate-500 uppercase">Verified Skill Profile</span>
                  <div className="flex flex-wrap gap-1.5 mt-1.5">
                    {analysisResult.parsedResume.skills.slice(0, 6).map((sk, idx) => (
                      <span key={idx} className="px-2 py-0.5 bg-slate-100 text-[10px] text-slate-700 rounded font-mono truncate max-w-[150px]">
                        {sk}
                      </span>
                    ))}
                    {analysisResult.parsedResume.skills.length > 6 && (
                      <span className="px-2 py-0.5 bg-slate-100 text-[10px] text-slate-500 rounded font-mono">
                ...
                <div className="pt-2">
                  <button
                    onClick={() => onNavigate("analyze")}
                    className="w-full py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-medium text-xs rounded-xl border border-slate-200 transition-all text-center uppercase font-mono tracking-wider"
                  >
                    Recalibrate Profile
                  </button>
                </div>
          ...
          <div className="bg-brand-card border border-slate-200 p-5 rounded-2xl space-y-3">
            <h5 className="font-display font-semibold text-slate-800 text-sm flex items-center">
